...
public class Q18_CombinationSumII {
    ...
    /**
     * so in CombinationSumII the only diffrence is we have to take the element one time we can not do it multiple times
     * and at the end while returning the solution we have to take care the combination that we make it should also be unique.
     ...
     * @param set
     */
    public static void findingCombinationSum(int arr[], int index, int target, List<Integer> current,HashSet<List<Integer>> set) {
        if (target==0) {
            List<Integer> temp = new ArrayList<>(current);
            //System.out.println("current putting in to temp "+temp+"    .");
            Collections.sort(temp);  // Sort to ensure unique order
            set.add(temp);  // Add to HashSet to filter duplicates
            return;
        }
 ...
 * OPTIMIZE WAY
 */
public static void findingCombinationSum_OptimizeWay(int arr[], int index, int target, List<Integer> current, List<List<Integer>> sol) {
    if (target == 0) {
